...
from scipy.signal import square , sawtooth , chirp , sweep_poly
...
def laser_sound():
    ...
    freq_range = (np.sin(2 * np.pi * freq_high * t) + 1) / 2 * (freq_high - freq_low)
...
def explosion_sound():
    ...
    freq_range = (np.sin(2 * np.pi * freq_high * t) + 1) / 2 * (freq_high - freq_low) - freq_high
...
def flying_noise():
    ...
    freq_range_after_burner = (np.sin(2 * np.pi * 1 * t_after_burner) + 1) / 2 * (freq_high - freq_low) + freq_low
    # Generate the  wave
    y = amplitude * square(2 * np.pi * freq_range * t)
    y_after_burner = amplitude * sawtooth(2 * np.pi * freq_range_after_burner * t_after_burner)
    ...
    convert_to_wav(sample_rate, y_norm, file_name_2)
    ...
    # Second subplot
    plt.subplot(2, 1, 2)
    plt.plot(t_after_burner, y_norm_after_burner)
    plt.title('Sine Wave (Copy)')
    plt.xlabel('Time (s)')
    plt.ylabel('Amplitude')
    plt.grid(True)
    plt.xlim([0, duration])
