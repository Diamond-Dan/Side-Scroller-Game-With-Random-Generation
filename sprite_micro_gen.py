...
def main(start_x,start_y,frames,seed,pixel_number,mode,wiggle,xml,pixel_size,file_name,server_mode):
    #for colin generate the food gif
    if mode == 'food':
        return generate_food_gifs(locals())
    int_x=start_x
    int_y=start_y
    filename=[]
    filename_2=[]
...
#Colin generate food gifs
def generate_food_gifs(data):
    start_x = data['start_x']
    start_y = data['start_y']
    frames = data['frames']
    seed = data['seed']
    pixel_number = data['pixel_number']
    # Assuming a new function for drawing food
    guide_array_x, guide_array_y, color_array = draw_food(start_x, start_y, seed, pixel_number)

    filenames = []
    gif_names = []

    for i in range(frames):
        filename, _ = draw_image_guided_food(guide_array_x, guide_array_y, color_array, i, data['wiggle'], data['pixel_size'])
        if filename:
            filenames.append(filename)

    # Generate GIF from the collected filenames
    gif_loc = gif_maker(filenames, os.path.dirname(os.path.realpath(__file__)), seed, pixel_number, frames)
    return filenames, gif_loc

#Colin drwa image guided
def draw_image_guided_food(guide_array_x, guide_array_y, color_array, frame_index, filename):
    # Create image using guide arrays and color
    img = Image.new('RGB', (200, 200), 'white')
    draw = ImageDraw.Draw(img)
    for x, y, color in zip(guide_array_x, guide_array_y, color_array):
        # Modify x, y based on frame_index to simulate motion or change
        draw.ellipse([x-5, y-5, x+5, y+5], fill=color)
    img.save(f'{filename}_{frame_index}.png')
    return f'{filename}_{frame_index}.png'

#colin draw_food
def draw_food(x, y, seed, pixel_number):
    # Simple drawing of an apple-like shape
    img = Image.new('RGBA', (100, 100), color=(255, 255, 255, 0))
    draw = ImageDraw.Draw(img)
    # Apple body
    draw.ellipse((x-30, y-30, x+30, y+30), fill=(255, 0, 0, 255))
    # Apple stalk
    draw.rectangle((x-5, y-40, x+5, y-30), fill=(139, 69, 19, 255))
    return img
...
def gif_maker(filename,cur_file_loc,seed,pixel_number,frames,filecount):
    ...
    gif_name = os.path.join(cur_file_loc, "gifs", "movie_"+"seed_"+str(seed)+"pixel_"+str(pixel_number)+"frames_"+str(frames)+str(filecount)+".gif")
...
def image_saver(img,file_name):
    ...
    server_name=""
    cur_file_loc = os.path.dirname(os.path.realpath(__file__))
    while os.path.isfile(os.path.join(cur_file_loc, "Images", file_name + str(filecount) + ".png")):
        filecount += 1
        name = os.path.join(cur_file_loc, "Images", file_name + str(filecount) + ".png")
    server_name = file_name + str(filecount) + ".png"
    print(server_name)

    img.save(os.path.join(cur_file_loc, "Images", file_name + str(filecount) + ".png")) 
