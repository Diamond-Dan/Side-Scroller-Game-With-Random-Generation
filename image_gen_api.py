...
def generate_random_images():
    ...
    image_urls = {
        'gif_loc_1': 'http://127.0.0.1:5000/gifs/' + gif_loc_1.replace("\\", "/"),
        'gif_loc_2': 'http://127.0.0.1:5000/gifs/' + gif_loc_2.replace("\\", "/")
    }
    for i, name in enumerate(filename, start=1):
        image_urls[f'filename_{i}'] = f'http://127.0.0.1:5000/images/{name}'.replace("\\", "/")
    for i, name in enumerate(filename_2, start=1):
        image_urls[f'filename_2_{i}'] = f'http://127.0.0.1:5000/images/{name}'.replace("\\", "/")
...
def generate_images_criteria():
    ...
    print(curfile)
    xml_file_path = os.path.join(curfile, 'patterns', xml)
    # Modify the existing line that incorrectly uses backslashes or forward slashes
    if not os.path.isfile(os.path.join(curfile, 'patterns', xml)):
